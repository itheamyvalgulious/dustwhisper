...
def ensure_world_resources(bridge, world: "WorldEngine") -> None:
    ...
    bridge.textures["ambient_temperature"] = bridge.ctx.texture(
    ...
    bridge.textures["visible_illumination"] = bridge.ctx.texture(
        (world.width, world.height), 4, dtype="f4"
    )
    bridge.textures["liquid_flow_intent"] = bridge.ctx.texture(
        (world.width, world.height), 2, dtype="f4"
    )
    for texture in bridge.textures.values():
        texture.filter = (bridge.ctx.NEAREST, bridge.ctx.NEAREST)
    bridge.buffers["cell_core"] = bridge.ctx.buffer(
        reserve=world.width * world.height * 5 * 4, dynamic=True
    )
    bridge.buffers["island_id"] = bridge.ctx.buffer(
        reserve=max(4, world.width * world.height * 4), dynamic=True
    )
    bridge.buffers["entity_id"] = bridge.ctx.buffer(
        reserve=max(4, world.width * world.height * 4), dynamic=True
    )
    ...
    bridge.buffers["entity_state"] = bridge.ctx.buffer(
        reserve=max(4, ENTITY_STATE_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["entity_state_count"] = bridge.ctx.buffer(reserve=4, dynamic=True)
    bridge.buffers["force_source"] = bridge.ctx.buffer(
        reserve=max(4, FORCE_SOURCE_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["force_source_count"] = bridge.ctx.buffer(reserve=4, dynamic=True)
    bridge.buffers["island_runtime"] = bridge.ctx.buffer(
        reserve=max(4, ISLAND_RUNTIME_DTYPE.itemsize), dynamic=True
    )
    ...
    bridge.buffers["world_command"] = bridge.ctx.buffer(
        reserve=max(4, WORLD_COMMAND_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["world_command_payload"] = bridge.ctx.buffer(reserve=4, dynamic=True)
    bridge.buffers["readback_request"] = bridge.ctx.buffer(
        reserve=max(4, READBACK_REQUEST_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["readback_request_label"] = bridge.ctx.buffer(reserve=4, dynamic=True)
    bridge.buffers["placeholder"] = bridge.ctx.buffer(
        reserve=max(4, PLACEHOLDER_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["placeholder_dirty_rect"] = bridge.ctx.buffer(
        reserve=max(4, PLACEHOLDER_DIRTY_RECT_DTYPE.itemsize),
        dynamic=True,
    )
    bridge.buffers["active_meta"] = bridge.ctx.buffer(
        reserve=max(4, ACTIVE_META_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["active_tile_ttl"] = bridge.ctx.buffer(
        reserve=max(4, world.active.tile_width * world.active.tile_height * 4), dynamic=True
    )
    ...
    bridge.buffers["active_chunk_list"] = bridge.ctx.buffer(
        reserve=max(8, active_chunk_count * 2 * 4), dynamic=True
    )
    ...
    bridge.buffers["gas_runtime_meta"] = bridge.ctx.buffer(
        reserve=max(4, GAS_RUNTIME_META_DTYPE.itemsize), dynamic=True
    )
    ...
    bridge.buffers["heat_runtime_meta"] = bridge.ctx.buffer(
        reserve=max(4, HEAT_RUNTIME_META_DTYPE.itemsize), dynamic=True
    )
    ...
    bridge.buffers["liquid_runtime_meta"] = bridge.ctx.buffer(
        reserve=max(4, LIQUID_RUNTIME_META_DTYPE.itemsize), dynamic=True
    )
    ...
    bridge.buffers["page_stripe_meta"] = bridge.ctx.buffer(
        reserve=max(4, PAGE_STRIPE_META_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["page_stripe_section"] = bridge.ctx.buffer(
        reserve=max(4, PAGE_STRIPE_SECTION_DTYPE.itemsize), dynamic=True
    )
    bridge.buffers["page_stripe_payload"] = bridge.ctx.buffer(reserve=4, dynamic=True)
    bridge.buffers["frame_meta"] = bridge.ctx.buffer(
        reserve=max(4, FRAME_META_DTYPE.itemsize), dynamic=True
    )
...
def ensure_cell_core_spare(bridge, world: "WorldEngine") -> Any:
    ...
    # Seed the spare from the live core: the provenance pass that publishes
    # through this buffer only guarantees coverage of the cells it touches,
    # so an uninitialised spare would leak stale driver memory into the live
    # core after the ping-pong swap (and into full-core downloads).
    live = bridge.buffers.get("cell_core")
    if live is not None and live is not spare and int(getattr(live, "size", 0)) >= required_size:
        bridge.ctx.copy_buffer(spare, live, required_size)
...
def _ensure_atlas_texture(bridge, world: "WorldEngine") -> None:
    ...
    for material in world.rulebook.materials_by_id.values():
        ...
        atlas[ty * tile : (ty + 1) * tile, tx * tile : (tx + 1) * tile] = _render_group_tile(
            material, tile
        )
    ...
    if existing is not None:
        try:
            ...
            pass
    bridge.textures["atlas"] = bridge.ctx.texture(
        (cols * tile, rows * tile), 3, atlas.tobytes(), dtype="f4"
    )
