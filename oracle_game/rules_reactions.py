...
def _build_rules(materials: Iterable[object] | None = None) -> dict[str, object]:
    ...
    for name, slot_index in acid_slot_map.items():
        MM.append(
            PairReactionRule(
                lhs_material=name, rhs_material="acid_liquid", trigger_slot_index=slot_index
            )
        )
    MM.append(
    ...
    for name, slot_index in poison_slot_map.items():
        MM.append(
            PairReactionRule(
                lhs_material=name, rhs_material="poison_liquid", trigger_slot_index=slot_index
            )
        )
        MG.append(
            PairReactionRule(
    ...
    for name, slot_index in pollution_slot_map.items():
        MM.append(
            PairReactionRule(
                lhs_material=name, rhs_material="pollution_powder", trigger_slot_index=slot_index
            )
        )
    MG.append(
        PairReactionRule(
            lhs_material="fire_powder", rhs_gas="oil_gas", threshold=0.18, result_action=7
        )
    )
    ...
    holy_restoration_materials = tuple(
        name for name in all_light_materials if name != "pollution_powder"
    )
    for name in all_light_materials:
        ML.append(
            PairReactionRule(
                lhs_material=name, rhs_light="visible_light", threshold=0.05, trigger_slot_index=6
            )
        )
        ML.append(
            PairReactionRule(
                lhs_material=name, rhs_light="holy_light", threshold=0.05, trigger_slot_index=7
            )
        )
        ML.append(
            PairReactionRule(
                lhs_material=name, rhs_light="chaos_light", threshold=0.05, trigger_slot_index=6
            )
        )
    for name in holy_restoration_materials:
        ML.append(
            PairReactionRule(
                lhs_material=name, rhs_light="holy_light", threshold=0.05, result_action=23
            )
        )
    ML.append(
        PairReactionRule(
            lhs_material="pollution_powder",
            rhs_light="holy_light",
            threshold=0.03,
            result_action=17,
        )
    )
    ML.append(
        PairReactionRule(
            lhs_material="placeholder_solid",
            rhs_light="chaos_light",
            threshold=0.05,
            result_action=11,
        )
    )
    for name in all_light_materials:
        ML.append(
            PairReactionRule(
                lhs_material=name, rhs_light="chaos_light", threshold=0.12, result_action=18
            )
        )
    ...
    SR.extend(
        [
            ...
            SelfReactionRule(
                material="explosive_solid", trigger_slot_index=0, min_temperature=90.0
            ),
            SelfReactionRule(
                material="explosive_solid", trigger_slot_index=1, min_temperature=90.0
            ),
            SelfReactionRule(
                material="explosive_solid", trigger_slot_index=4, min_temperature=90.0
            ),
