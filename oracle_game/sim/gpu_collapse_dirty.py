...
# Substitution table for the ``{{NAME}}`` markers used by the external
# ``shaders/collapse/dirty_*.comp`` files.  Compound f-string interpolations
# (``LOCAL_SIZE - 1``) were baked in as literal values during extraction,
# matching the convention used by the other collapse shaders.
...
COLLAPSE_STRUCTURE_ACTIVE_TILE_DISPATCH_ARGS_BUFFER = (
    "collapse_structure_dirty_active_tile_dispatch_args"
)
COLLAPSE_STRUCTURE_ACTIVE_TILE_FLAGS_BUFFER = "collapse_structure_dirty_active_tile_flags"
COLLAPSE_STRUCTURE_GUARDED_DIRTY_DISPATCH_ARGS_BUFFER = (
    "collapse_structure_guarded_dirty_dispatch_args"
)
...
def get_collapse_structure_dirty_tile_bounds(
    world: "WorldEngine",
...
def _active_source_tile_bounds(
    ...
    *,
    ...
    if not isinstance(chunk_source, np.ndarray) or chunk_source.shape != (
        chunk_height,
        chunk_width,
    ):
...
def _shader_storage_barrier(ctx: Any, *, command: bool = False) -> None:
    flags = getattr(ctx, "SHADER_STORAGE_BARRIER_BIT", 0) | getattr(
        ctx, "BUFFER_UPDATE_BARRIER_BIT", 0
    )
    if command:
...
def clear_collapse_structure_dirty_tile_mask(world: "WorldEngine") -> None:
    ...
    for name, payload in (
        ...
        (
            ...
            np.zeros(tile_count * 2, dtype=np.int32).tobytes(),
        ),
        (
...
def ensure_collapse_structure_dirty_tile_queue(
    ...
    *,
    ...
    specs = (
        ...
        (
            COLLAPSE_STRUCTURE_DIRTY_TILE_LIST_BUFFER,
            max(8, tile_count * 2 * np.dtype(np.int32).itemsize),
        ),
...
def _material_participation_flags(world: "WorldEngine") -> np.ndarray:
    ...
    valid_rows = (
        material_table["name_hash"] != 0
        if "name_hash" in material_table.dtype.names
        else np.ones_like(structural)
    )
    valid_participating = valid_rows & (material_ids >= 0)
    ids = material_ids[valid_participating]
    flags[ids] = structural[valid_participating].astype(np.uint32, copy=False) | (
...
def _ensure_active_tile_dispatch_buffers(world: "WorldEngine") -> tuple[Any, Any, Any, Any]:
    ...
    specs = (
        ...
        (
            ...
            max(4, tile_count * np.dtype(np.uint32).itemsize),
        ),
    )
    buffers: dict[str, Any] = {}
    for name, required_bytes in specs:
...
def _collapse_dirty_program(world: "WorldEngine", key: str, rel_path: str) -> Any:
    bridge = world.bridge
    ctx = bridge.ctx
    if ctx is None:
        raise RuntimeError("GPU collapse dirty tracking requires bridge GPU resources")
    ...
    if program is not None:
        return program
    program = build_compute_shader(ctx, rel_path, _SHADER_SUBS)
...
def _clear_active_tile_dispatch_program(world: "WorldEngine") -> Any:
    return _collapse_dirty_program(
        ...
        "collapse/dirty_clear_active_tile_dispatch.comp",
    )
...
def _compact_active_tiles_from_chunks_program(world: "WorldEngine") -> Any:
    return _collapse_dirty_program(
        ...
        "collapse/dirty_compact_active_tiles_from_chunks.comp",
    )
...
def _dirty_mark_program(world: "WorldEngine", *, packed_cell_state: bool = False) -> Any:
    return _collapse_dirty_program(
        world,
        "collapse_structure_dirty_tiles_packed"
        if packed_cell_state
        else "collapse_structure_dirty_tiles",
        "collapse/dirty_mark_tiles_packed.comp"
        if packed_cell_state
        else "collapse/dirty_mark_tiles.comp",
    )
...
def _guarded_dirty_dispatch_args(world: "WorldEngine", guard_buffer: Any, source_args: Any) -> Any:
    ...
    program = _collapse_dirty_program(
        ...
        "collapse/dirty_guard_dispatch_args.comp",
    )
...
def _clear_dirty_tile_queue_program(world: "WorldEngine") -> Any:
    return _collapse_dirty_program(
        ...
        "collapse/dirty_clear_tile_queue.comp",
    )
...
def _mark_dirty_tile_rect_program(world: "WorldEngine") -> Any:
    return _collapse_dirty_program(
        ...
        "collapse/dirty_mark_tile_rect.comp",
    )
...
def _build_active_tile_dispatch(
    ...
    *,
    ...
    active_count, active_list, active_dispatch_args, active_flags = (
        _ensure_active_tile_dispatch_buffers(world)
    )
    tile_count = max(1, int(world.active.tile_width) * int(world.active.tile_height))
    ...
    if not hasattr(compact_program, "run_indirect"):
        raise RuntimeError(
            "GPU collapse dirty active tile compaction requires ComputeShader.run_indirect"
        )
...
def mark_collapse_structure_dirty_tiles_from_bridge_cell_core(
    ...
    *,
    ...
    if not packed_cell_state and (material_texture is None or phase_texture is None):
        raise ValueError(
            "collapse dirty tracking requires split material/phase or packed cell state"
        )
    ...
    if dispatch_guard_buffer is not None:
        dispatch_args = _guarded_dirty_dispatch_args(
            world, dispatch_guard_buffer, active_dispatch_args
        )
