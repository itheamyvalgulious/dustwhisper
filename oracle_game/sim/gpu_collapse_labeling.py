...
def label_component_mask(
    ...
    *,
    ...
    label_texture, width, height = pipeline._label_component_mask_texture(
        world, component_mask, x0=x0, y0=y0
    )
    if width == 0 or height == 0:
        return np.zeros_like(component_mask, dtype=np.int32)
    return np.rint(np.frombuffer(label_texture.read(), dtype="f4").reshape((height, width))).astype(
        np.int32
    )
...
def materialize_component_texture_formal(
    ...
    *,
    ...
    if not pipeline._formal_gpu_frame(world):
        raise RuntimeError(
            "formal component texture materialization requires an active formal GPU frame"
        )
    if width == 0 or height == 0:
        return 0
    ...
    *,
    ...
    if pipeline._formal_gpu_frame(world) and tile_mask_name is not None:
        ...
        if not hasattr(program, "run_indirect"):
            raise RuntimeError(
                "formal connected component collect requires ComputeShader.run_indirect"
            )
...
def _clear_component_label_flags_connected_tiles(
    ...
    if not hasattr(program, "run_indirect"):
        raise RuntimeError(
            "formal connected component flag clear requires ComputeShader.run_indirect"
        )
    program["cell_grid_size"].value = (int(width), int(height))
    program["tile_grid_size"].value = (
        int(getattr(world.active, "tile_width", 1)),
    ...
    *,
    ...
    with pipeline._profile_pass(world, "label_collect_components.collect_roots"):
        ...
        if component_flag_generation is not None:
            collect_kwargs["component_flag_generation"] = int(component_flag_generation)
...
def _summarize_formal_component_metadata(
    ...
    *,
    ...
    if pipeline._formal_gpu_frame(world) and tile_mask_name is not None:
        ...
        summarize_program.run_indirect(
...
def _materialize_compact_labeled_component_texture(
    ...
    *,
    ...
    with pipeline._profile_pass(world, "materialize.main"):
        ...
        collapse_generation, base_integrity, spawn_temperature = (
            pipeline._materialize_material_params(world)
        )
        ...
        pipeline._write_dynamic_buffer(
            ctx, resources, "material_spawn_temperature", spawn_temperature
        )

        connected_tiles = pipeline._formal_gpu_frame(world) and tile_mask_name is not None
        program = pipeline.programs[
            "materialize_compact_components_connected_tiles"
            if connected_tiles
            else "materialize_compact_components"
        ]
        if connected_tiles:
            program["cell_grid_size"].value = (int(width), int(height))
            program["tile_grid_size"].value = (
        ...
        if connected_tiles:
            ...
            world.bridge.buffers[tile_mask_name].bind_to_storage_buffer(binding=4)
            world.bridge.buffers[FORMAL_CONNECTED_TILE_COUNT_BUFFER].bind_to_storage_buffer(
                binding=5
            )
            world.bridge.buffers[FORMAL_CONNECTED_TILE_LIST_BUFFER].bind_to_storage_buffer(
                binding=6
            )
            program.run_indirect(world.bridge.buffers[FORMAL_CONNECTED_TILE_DISPATCH_ARGS_BUFFER])
        else:
...
def _publish_compact_component_island_runtime(
    ...
    *,
    ...
    dispatch_invocations = (
        256 * dispatch_stride if pipeline._runtime_admission_stride_dispatch_enabled else 256
    )
...
def _publish_dense_component_island_runtime(
    ...
    if ctx is None:
        raise RuntimeError("GPU collapse pipeline requires a valid ModernGL context")
    pipeline._summarize_dense_component_metadata(
        world, label_texture, component_capacity, x0, y0, width, height
    )
    bridge = world.bridge
    bridge.ensure_world_resources(world)
    if not bridge.enabled or bridge.ctx is None:
...
def _label_component_mask_texture(
    ...
    *,
    ...
    return pipeline._label_component_texture(
        world, resources.structural_tex, width, height, x0=x0, y0=y0
    )
...
def collect_component_labels(
    ...
    component_count = int(
        np.frombuffer(resources.component_count.read(size=4), dtype=np.uint32, count=1)[0]
    )
