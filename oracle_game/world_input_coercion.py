...
def _coerce_material_def(engine, material: MaterialDef | dict[str, Any]) -> MaterialDef:
    ...
    payload["reaction_slots"] = tuple(
        payload.get("reaction_slots", (-1, -1, -1, -1, -1, -1, -1, -1))
    )
    payload["tags"] = tuple(payload.get("tags", ()))
    payload["collapse_generation"] = _canonical_material_input_name(
        payload.get("collapse_generation")
    )
    ...
    payload["freeze_to_material"] = _canonical_material_input_name(
        payload.get("freeze_to_material")
    )
...
def _coerce_gas_species_def(engine, gas: GasSpeciesDef | dict[str, Any]) -> GasSpeciesDef:
    ...
    payload["condense_to_material"] = _canonical_material_input_name(
        payload.get("condense_to_material")
    )
...
def _coerce_material_optics_def(
    engine, optics: MaterialOpticsDef | dict[str, Any]
) -> MaterialOpticsDef:
...
def _coerce_reaction_rules(engine, rules: dict[str, object]) -> dict[str, list[object]]:
    return {
        "material_material": [
            _coerce_pair_reaction_rule(engine, rule) for rule in rules.get("material_material", [])
        ],
        "material_gas": [
            _coerce_pair_reaction_rule(engine, rule) for rule in rules.get("material_gas", [])
        ],
        "material_light": [
            _coerce_pair_reaction_rule(engine, rule) for rule in rules.get("material_light", [])
        ],
        "gas_gas": [_coerce_pair_reaction_rule(engine, rule) for rule in rules.get("gas_gas", [])],
        "gas_light": [
            _coerce_pair_reaction_rule(engine, rule) for rule in rules.get("gas_light", [])
        ],
        "self_rules": [
            _coerce_self_reaction_rule(engine, rule) for rule in rules.get("self_rules", [])
        ],
    }
...
def _frame_force_source_input(engine, force_source: ForceSource | dict[str, Any]) -> ForceSource:
    ...
    if force_source.world_x is not None and force_source.world_y is not None:
        return replace(
            force_source, world_x=float(force_source.world_x), world_y=float(force_source.world_y)
        )
    world_x, world_y = engine._buffer_to_world_float_position(
        (float(force_source.x), float(force_source.y))
    )
...
def _coerce_entity_placeholder(
    engine, placeholder: EntityPlaceholder | dict[str, Any]
) -> EntityPlaceholder:
...
def _frame_entity_placeholder_input(
    ...
    placeholder = _coerce_entity_placeholder(engine, placeholder)
    if placeholder.world_x is not None and placeholder.world_y is not None:
        buffer_x, buffer_y = engine._world_to_buffer_clamped(
            int(placeholder.world_x), int(placeholder.world_y)
        )
        world_x = int(placeholder.world_x)
        world_y = int(placeholder.world_y)
    else:
...
def _coerce_entity_observation_spec(
    ...
    return EntityObservationSpec(
        ...
        observe_width=None
        if payload.get("observe_width") is None
        else int(payload["observe_width"]),
        observe_height=None
        if payload.get("observe_height") is None
        else int(payload["observe_height"]),
        observe_label=None
        if payload.get("observe_label") is None
        else str(payload["observe_label"]),
    )
...
def _normalize_entity_state_patch_fields(engine, fields: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, value in fields.items():
        if (
            name not in ENTITY_STATE_PATCHABLE_FIELDS
            and name not in ENTITY_STATE_PATCH_METADATA_FIELDS
        ):
            raise KeyError(name)
...
def _public_entity_state_patch_input(
    ...
    return EntityStatePatch(
        entity_id=int(patch.entity_id), fields=_normalize_entity_state_patch_fields(engine, fields)
    )
...
def _frame_entity_state_patch_input(
    ...
    if "_world_x" in fields:
        buffer_x, _ = engine._world_to_buffer_clamped(
            int(fields["_world_x"]), int(fields.get("_world_y", 0))
        )
        fields["x"] = int(buffer_x)
    if "_world_y" in fields:
        _, buffer_y = engine._world_to_buffer_clamped(
            int(fields.get("_world_x", 0)), int(fields["_world_y"])
        )
...
def _coerce_entity_state_patch(
    engine, patch: EntityStatePatch | dict[str, Any]
) -> EntityStatePatch:
...
def _coerce_observation_target(
    engine, target: ObservationTarget | dict[str, Any]
) -> ObservationTarget:
...
def _coerce_target_query(engine, query: TargetQuery | dict[str, Any]) -> TargetQuery:
    ...
    return TargetQuery(
        ...
        if payload.get("source_entity_id") is None
        else int(payload["source_entity_id"]),
        ...
        if payload.get("anchor_entity_id") is None
        else int(payload["anchor_entity_id"]),
        ...
        else float(payload["distance_meters"]),
        distance_hint=None
        if payload.get("distance_hint") is None
        else str(payload["distance_hint"]),
...
def _coerce_change_intent(engine, intent: ChangeIntent | dict[str, Any]) -> ChangeIntent:
    ...
    return ChangeIntent(
        ...
        else str(_canonical_material_input_name(payload["material"])),
        temperature_delta=float(payload.get("temperature_delta", 0.0)),
        velocity=None
        if payload.get("velocity") is None
        else tuple(float(value) for value in payload["velocity"]),
...
def _coerce_readback_request(engine, request: ReadbackRequest | dict[str, Any]) -> ReadbackRequest:
    ...
    return _normalize_readback_request(
        engine,
        ReadbackRequest(
...
def _assign_preview_readback_request_ids(
    ...
    *,
    ...
    predicted_next = int(
        engine.next_readback_request_id if next_request_id is None else next_request_id
    )
...
def _normalize_json_payload_value(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {str(key): _normalize_json_payload_value(item) for key, item in value.items()}
    return deepcopy(value)


def _coerce_world_frame_input(
    engine, frame_input: WorldFrameInput | dict[str, Any]
) -> WorldFrameInput:
    if isinstance(frame_input, WorldFrameInput):
        ...
        return WorldFrameInput(
            submission_id=None
            if frame_input.submission_id is None
            else int(frame_input.submission_id),
            focus_center=(
                None
            ...
            if frame_input.entity_placeholders is None
                ...
                _public_entity_placeholder_input(engine, item)
                for item in frame_input.entity_placeholders
            ],
            force_sources=None
            if frame_input.force_sources is None
            else [
                _public_force_source_input(engine, source) for source in frame_input.force_sources
            ],
            emitters=None
            if frame_input.emitters is None
            else [_coerce_emitter(engine, emitter) for emitter in frame_input.emitters],
            target_queries=[
                _coerce_target_query(engine, query) for query in frame_input.target_queries
            ],
            change_intents=[
                _coerce_change_intent(engine, intent) for intent in frame_input.change_intents
            ],
            carrier_intents=[
                _coerce_carrier_intent(engine, intent) for intent in frame_input.carrier_intents
            ],
            observation_targets=[
                _coerce_observation_target(engine, target)
                for target in frame_input.observation_targets
            ],
            readback_requests=[
                _coerce_readback_request(engine, request)
    ...
    controller_state_provided = (
        bool(payload.get("controller_state_provided", False)) or "controller_state" in payload
    )
    return WorldFrameInput(
        submission_id=None
        if payload.get("submission_id") is None
        else int(payload["submission_id"]),
        ...
        if payload.get("entity_placeholders") is None
            ...
            _public_entity_placeholder_input(engine, item)
            for item in payload.get("entity_placeholders", [])
        ],
        force_sources=None
        if payload.get("force_sources") is None
        else [
            _public_force_source_input(engine, source)
            for source in payload.get("force_sources", [])
        ],
        emitters=None
        if payload.get("emitters") is None
        else [_coerce_emitter(engine, emitter) for emitter in payload.get("emitters", [])],
        target_queries=[
            _coerce_target_query(engine, query) for query in payload.get("target_queries", [])
        ],
        change_intents=[
            _coerce_change_intent(engine, intent) for intent in payload.get("change_intents", [])
        ],
        carrier_intents=[
            _coerce_carrier_intent(engine, intent) for intent in payload.get("carrier_intents", [])
        ],
        observation_targets=[
            _coerce_observation_target(engine, target)
            for target in payload.get("observation_targets", [])
        ],
        readback_requests=[
            _coerce_readback_request(engine, request)
            for request in payload.get("readback_requests", [])
        ],
        commands=[
            _coerce_world_command(engine, command) for command in payload.get("commands", [])
        ],
    )
