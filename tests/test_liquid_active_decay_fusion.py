...
def test_flow_active_decay_is_one_tile_leader_after_terminal_publish() -> None:
    ...
    assert source.index("decay_active_tile_once(gid, group_origin)") > source.index(
...
def test_optics_uses_current_frame_liquid_snapshot_only_after_fusion() -> None:
    source = shader_source(
        ...
        {
            "ACTIVE_TILE_SNAPSHOT": 1,
            "LOCAL_SIZE": 8,
            "LIGHT_PARAM_COUNT": 16,
            "MAX_EMITTERS": 256,
            "MAX_LIGHTS": 8,
        },
    )
...
def test_world_active_decay_keeps_standalone_fallback() -> None:
    ...
    assert (
        "if not active_decay_fused and not engine.bridge.decay_active_scheduler(engine)" in source
    )
