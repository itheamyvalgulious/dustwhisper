    ...
    *,
    ...
    try:
        ...
        engine.cell_temperature[:] = rng.normal(
            25.0, 30.0, size=engine.cell_temperature.shape
        ).astype(np.float32)
        engine.integrity[:] = rng.uniform(2.0, 100.0, size=engine.integrity.shape).astype(
            np.float32
        )
        engine.timer_pack.fill(0)
        engine.gas_concentration[:] = rng.uniform(
            0.0, 0.3, size=engine.gas_concentration.shape
        ).astype(np.float32)
...
def test_self_rule_material_span_metadata_is_grouped_and_cache_invalidates() -> None:
    ...
    try:
        ...
        def read_metadata() -> tuple[np.ndarray, np.ndarray]:
            ...
            compiled = (
                np.frombuffer(resources.self_rule_i.read(), dtype=np.int32).reshape((-1, 4)).copy()
            )
            material_tags = (
